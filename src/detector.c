...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers, char* result_path, int override_img)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    int names_size = 0;
    ...
    while (1) {
        ...
        draw_detections_v3(im, dets, nboxes, thresh, names, alphabet, l.classes, ext_output, image_name, result_path);
        ...
        if (override_img) {
            ...
            save_image(im, "predictions.jpg"); // hardcoded .jpg ??
        }
...
void run_detector(int argc, char **argv)
{
    ...
    char* result_path = find_char_arg(argc, argv, "-result_path", 0); 
    int override_img = find_arg(argc, argv, "-override_img");
    ...
    if (0 == strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers, result_path, override_img);
